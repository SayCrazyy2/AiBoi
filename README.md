...
Use the built-in `ai mcp` commands to browse a curated catalog and add
servers without editing JSON by hand:

```bash
ai mcp list               # show all 16 available servers (catalog)
ai mcp installed          # show servers in your config
ai mcp add <name>         # add a server (interactive -- prompts for paths/keys)
ai mcp add-custom         # add any custom server not in the catalog
ai mcp remove <name>      # remove a server from your config
ai mcp install <name>     # pre-download a server package (npx/uvx cache)
```
...
| Server | Via | Description |
|--------|-----|-------------|
| filesystem | npx | Secure file read/write with configurable access |
| fetch | uvx | Web content fetching and conversion |
| git | uvx | Read, search, and manipulate Git repositories |
| memory | npx | Knowledge graph-based persistent memory |
| sequential-thinking | npx | Dynamic and reflective problem-solving |
| time | uvx | Time and timezone conversion |
| puppeteer | npx | Browser automation and web scraping |
| everything | npx | Reference/test server with prompts, resources, and tools |
| sqlite | uvx | SQLite database interaction |
| postgres | npx | PostgreSQL read-only database access |

**API key required:**

| Server | Via | Description |
|--------|-----|-------------|
| github | npx | GitHub repo management and API integration |
| brave-search | npx | Web and local search via Brave's API |
| google-drive | npx | File access and search for Google Drive |
| slack | npx | Slack channel management and messaging |
| google-maps | npx | Google Maps location services |
| everart | npx | AI image generation |

You can still edit `~/.ai-cli/mcp_servers.json` directly too:
...
from behind NAT/mobile data. Messages are sent with `parse_mode=MarkdownV2`
for rich formatting, with a "thinking…" indicator while the AI is working,
and a tool-call log footer showing which tools/MCP servers were used.
...
**Tool-call logging**: every bot response includes a footer like:
```
---
🔧 Tools used: 2  ⏱ 3.2s
✅ `read_file`(path=/data/info.txt) → First 100 chars of file…
✅ `mcp__fetch__fetch`(url=https://example.com) → Page content…
   ⏱ 1.5s
📦 MCP servers: fetch(1)
```
...
```
ai_cli/
  cli.py               argparse entrypoint, one-shot mode, `ai bots ...` / `ai mcp ...` / `ai serve` routing
  repl.py               interactive REPL + slash commands
  wizard.py             first-run / `ai --setup` interactive setup wizard
  mcp_tool.py            `ai mcp` subcommand: catalog, add, remove, install, add-custom
  session.py             conversation state + agentic tool-calling loop (with per-tool timing)
  ...
  bots/
    ...
    telegram_bot.py            long-polling Telegram frontend (MarkdownV2, thinking indicator, streaming)
    discord_bot.py             discord.py gateway frontend

setup.sh                  platform-detecting install + service wizard (VPS/macOS/local)
setup_mcp.sh               install Node.js + uv and pre-cache MCP server packages
mcp_servers.json            default MCP servers config (16 servers)
