...
def main(argv=None) -> int:
    ...
    if first_run and not args.list_models:
        if cfgmod.is_noninteractive():
            # Railway / Docker / systemd / CI: nobody's here to answer
            # prompts. Build the config from environment variables instead
            # of launching the wizard.
            console.print(
                "[dim]No config file and no interactive terminal detected -- "
                "building config from environment variables.[/dim]"
            )
            cfg = cfgmod.config_from_env()
        else:
...
def _cmd_serve(console: Console) -> int:
    """
    `ai serve` -- the entrypoint meant for Railway/Docker/systemd. Loads
    config from a file if present, otherwise straight from environment
    variables. Binds a health-check endpoint on $PORT if the platform set
    one. Starts whichever bots have tokens configured; if none do, stays
    up (instead of exiting/crash-looping) so you can see logs, set env
    vars, and redeploy.
    """
    cfg = cfgmod.load_config_auto()
    ...
    if port_str.isdigit():
        ...
        start_health_server(int(port_str))
    ...
    if not tg_token and not dc_token:
        console.print(
            "[yellow]No bot tokens configured.[/yellow] Set TELEGRAM_BOT_TOKEN and/or "
            "DISCORD_BOT_TOKEN as environment variables and redeploy, or run "
            "`ai bots setup` in a shell on this host.\n"
            "Staying up (health check still responds) so this doesn't crash-loop..."
        )
        try:
